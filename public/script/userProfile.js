    ...
    if (hasMorePages){
       $.get('/administer/user?page='+(lastPageId+1)+'&limit='+limit, function(data) {
            $.each(data.users, function(index, element) {           
                ...
                var privilageLevel = (element.privilage == 2 ? 'Admin' : (element.privilage == 1 ? 'Coordinator' : 'Citizen'));
...
$('#userProfile').on('click', '.editProfile', function() {
    ...
    var privilage = '<td><div class="btn-group open"><button style="font-size: 12px;" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="btn btn-default btn-xs dropdown-toggle"><span class="caret"></span><span id="role-option-'+id+'" option="'+role+'"> '+ (role == 2 ? 'Admin' : (role == 1 ? 'Coordinator' : 'Citizen')) +'</span></button>';
    privilage += '<ul style="font-size: 12px;" class="dropdown-menu"><li><a class="role-option" userId="'+id+'" option="2"> Admin</a></li><li><a class="role-option" userId="'+id+'" option="1"> Coordinator</a></li><li><a class="role-option" userId="'+id+'" option="0"> Citizen</a></li></ul></div></td>';
    ...
    $.ajax({
        ...
        statusCode: {
            200: function(data) {
                ...
                password = '******';
                var role = (privilage == '2' ? 'Admin' : (privilage == '1' ? 'Coordinator' : 'Citizen'));
